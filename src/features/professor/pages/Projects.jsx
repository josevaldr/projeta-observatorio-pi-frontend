...
const MOCK_PROJECTS = [
  {
    ...
    link: "https://github.com/projeto/gestao-escolar",
    evaluation: null,
  },
  {
    id: 2,
    title: "App de Doação de Sangue",
    teamName: "Save Lives",
    class: "TADS25.103",
    uploadDate: "2023-10-30",
    dueDate: "2023-11-01",
    status: "avaliado",
    link: "https://github.com/projeto/doacao-sangue",
    evaluation: { score: 9.5, comments: "Excelente projeto! Bem estruturado." },
  },
  {
    id: 3,
    title: "IoT na Agricultura Familiar",
    teamName: "AgroTech",
    class: "TSI24.201",
    uploadDate: "2023-11-05",
    dueDate: "2023-11-03",
    status: "aguardando_avaliacao",
    link: "https://github.com/projeto/agro-iot",
    evaluation: null,
  },
];
...
export default function Projects() {
  ...
  const classesList = ["Todas", ...new Set(projects.map((p) => p.class))];

  const filteredProjects = useMemo(() => {
    if (filterClass === "Todas") return projects;
    return projects.filter((p) => p.class === filterClass);
      ...
      projects.map((p) =>
        p.id === selectedProject.id
          ? {
              ...p,
              status: "avaliado",
              evaluation: { score: parseFloat(score), comments },
            }
          : p
      )
    );
  ...
  const handleOpenDetails = (project) => {
    setProjetoDetalhado({
      ...
      descricao: "Descrição do projeto não disponível neste mock para o professor. Mas o projeto está relacionado à turma " + project.class + ".",
      ...
      equipe: {
        ...
        alunos: ["Aluno 1 (Mock)", "Aluno 2 (Mock)"],
      },
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {filteredProjects.length === 0 && (
          <div className="col-span-full py-12 text-center text-gray-500 bg-white border border-gray-100 rounded-2xl">
            Nenhum projeto encontrado para esta turma.
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleEvaluate} className="p-6 flex flex-col gap-5">
              ...
              <div className="flex gap-3 pt-2">
                <Button type="submit" variant="primary" fullWidth={false} className="flex-1">
                  Salvar Avaliação
                </Button>
                <Button
                  type="button"
